...
final class WordCounts {
  ...
  static Map<String, Integer> sort(Map<String, Integer> wordCounts, int popularWordCount) {
    return wordCounts.entrySet().stream().filter(Objects::nonNull).sorted(new WordCountComparator())
            .limit(Math.min(popularWordCount, wordCounts.size()))
            .collect(Collectors.toMap(
                    Map.Entry::getKey, Map.Entry::getValue, (k, v) -> k, LinkedHashMap::new));
  }
  // TODO: Reimplement this method using only the Stream API and lambdas and/or method references.done
